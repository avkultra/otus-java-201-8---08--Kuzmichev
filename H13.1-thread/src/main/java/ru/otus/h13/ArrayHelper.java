...
public class ArrayHelper {
    ...
    public ArrayHelper(int data_size, int countThread) {
        this.DATA_SIZE = data_size;
        MAX_THREADS = countThread;

        this.data1 = createArray(data_size);
    }
    ...
    private static int checkThreads(int sizeData, int threadsCount) {
        ...
        if (sizeData < DEL) return 1;

        while (sizeData / DEL < threadsCount) {
            threadsCount = threadsCount / 2;
        }
        return threadsCount;
    }

    private static int[] mergeSorted(int[] src1, int[] src2) {
